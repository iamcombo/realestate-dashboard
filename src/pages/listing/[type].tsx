...
const Listing = ({ data }: { data: IMintData[] }) => {
  ...
  const updateJson = async (sig: string) => {
    try {
  ...
  const { execute: handleSign, status } = useAsync(
    Sign,
    async (sig) => {
      await updateJson(sig);
    },
    (err) => {
      setModalConfirm(false);
      notifications.show({
        title: 'Notification',
        message: err?.reason || 'Something went wrong',
        color: 'red',
      });
    },
    false
